...
class Checker {
	...
	function getField(t:TType, f:String, e:Expr, forWrite = false) {
		function resolveMember(field:CField)
			return field.name == f
				&& (args == null || // if there were arguments (e.g. from an ECall expression), do method overload resolution..
					switch field.t {
	...
	function typeExpr(expr:Expr, withType:WithType, ?args:Array<TType>, ?ret:TType):TType {
		...
		switch (edef(expr)) {
			...
			case EField(o, f):
				...
				if (globals.exists(typeName))
					return typeField(EIdent(typeName).mk(expr), f, expr, false, args, retType);
				else
					return typeField(o, f, expr, false, args, retType);
			case ECheckType(v, t):
